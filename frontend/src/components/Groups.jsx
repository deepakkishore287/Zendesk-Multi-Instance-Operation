...
export default function Groups() {
  ...
  // PROD‑only Jira gating (same as your existing behavior)
  ...
  async function load() {
    ...
    try {
      const data = await api.listGroups(businessArea, environment)
  ...
  async function onCreate() {
    try {
      await api.createGroup(businessArea, environment, jiraTicket, {
        name: newName,
        description: newDesc
      })
  ...
  async function onUpdate(id, name, description) {
    try {
      await api.updateGroup(businessArea, environment, jiraTicket, id, { name, description })
      await load()
    } catch (e) { setError(e.message) }
  }
          ...
          <>
            <table className="table">
              ...
              <tbody>
                ...
                {pagedItems.length === 0 && (
                  <tr>
                    <td colSpan={4} className="muted">No results found</td>
                  </tr>
                )}
              </tbody>
            </table>
...
function GroupRow({ g, onUpdate, disabled }) {
  const [name, setName] = useState(g.name || '')
  const [desc, setDesc] = useState(g.description || '')
  const changed = name !== (g.name || '') || desc !== (g.description || '')
